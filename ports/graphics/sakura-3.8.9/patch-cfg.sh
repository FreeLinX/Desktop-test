...
sed -i 's#^INCLUDE_DIRECTORIES .*#INCLUDE_DIRECTORIES (. ${GTK_INCLUDE_DIRS} ${VTE_INCLUDE_DIRS} /home/devuan/FreeLinX/desktop-test/ports/build/deps/pcre2-10.48/include)#' CMakeLists.txt
sed -i 's#^LINK_LIBRARIES .*#LINK_LIBRARIES (-Wl,--start-group ${GTK_LIBRARIES} ${VTE_LIBRARIES} ${X11_LIBRARIES} gdk-3 atk-1.0 cairo cairo-gobject dl epoxy expat ffi fontconfig freetype fribidi gdk_pixbuf-2.0 gio-2.0 glib-2.0 gmodule-2.0 gobject-2.0 gtk-3 harfbuzz vte-2.91 pango-1.0 pangocairo-1.0 pangoft2-1.0 pcre2-8 pixman-1 png16 wayland-client wayland-cursor wayland-egl xkbcommon z lz4 m c++ c++abi unwind -Wl,--end-group)#; s#^LINK_DIRECTORIES .*#LINK_DIRECTORIES (${GTK_LIBRARY_DIRS} ${VTE_LIBRARY_DIRS} ${X11_LIBRARY_DIRS} /home/devuan/FreeLinX/desktop-test/ports/build/deps/wayland/lib /home/devuan/FreeLinX/desktop-test/ports/build/deps/libxkbcommon/lib)#' CMakeLists.txt
sed -i '/#include <gdk\/gdkx.h>/d' src/sakura.c
cp /home/devuan/FreeLinX/desktop-test/ports/graphics/sakura-3.8.9/flx_cxa_thread_atexit.c src/flx_cxa_thread_atexit.c
